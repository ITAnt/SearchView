...
public class MaterialSearchView extends FrameLayout implements View.OnClickListener, onSearchActionsListener {
    ...
    private void openKeyboard(){
        new Handler().postDelayed(new Runnable() {
            public void run() {
                getSearchView().dispatchTouchEvent(MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_DOWN, 0, 0, 0));
                getSearchView().dispatchTouchEvent(MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_UP, 0, 0, 0));
            }
        }, 200);
    }
